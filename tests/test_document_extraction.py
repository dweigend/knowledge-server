...
@pytest.fixture
def source(application, article):
...
def job_state(ledger, request_hash):
...
def test_snapshots_pin_exact_source_and_extraction_revisions(
    application, source, snapshot, monkeypatch
):
    with application.database.transaction() as ledger:
...
def test_snapshot_rows_are_immutable(application, source, snapshot, statement):
    with application.database.transaction() as ledger:
        request_hash = extraction.request_extraction(ledger, source[0])
        extraction.claim_job(ledger)
        extraction.save_snapshot(ledger, request_hash, snapshot)
...
def test_interrupted_jobs_recover_only_within_attempt_budget(application, source):
    with application.database.transaction() as ledger:
        request_hash = extraction.request_extraction(ledger, source[0])
...
def test_failures_retry_transient_jobs_once(application, source, transient):
    with application.database.transaction() as ledger:
        request_hash = extraction.request_extraction(ledger, source[0])
        extraction.claim_job(ledger)
...
def test_snapshot_rejects_mismatched_identity_or_unclaimed_job(
    application, source, snapshot, mismatch
):
    with application.database.transaction() as ledger:
        request_hash = extraction.request_extraction(ledger, source[0])
        if mismatch != "queued":
...
def test_failure_cannot_overwrite_completed_job(application, source, snapshot):
    with application.database.transaction() as ledger:
        request_hash = extraction.request_extraction(ledger, source[0])
        extraction.claim_job(ledger)
        extraction.save_snapshot(ledger, request_hash, snapshot)
...
def test_page_map_verifies_renders_for_identical_or_cover_removed_pdfs(tmp_path, remove_cover):
...
def test_page_map_rejects_swapped_pages_with_equal_page_counts(tmp_path):
    original = write_pdf(tmp_path / "original.pdf", [0.1, 0.4, 0.8])
...
def test_page_map_rejects_unexplained_removed_pages(tmp_path):
    original = write_pdf(tmp_path / "original.pdf", [0.1, 0.4, 0.8])
