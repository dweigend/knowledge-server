"""Execute the fixed Docling/Marker workflow without holding knowledge transactions."""
...
from knowledge.contracts import Reference, Source
...
from knowledge.document_extraction import (
    CONFIGURATION,
    claim_job,
    fail_job,
    recover_jobs,
...
from knowledge.extraction_tools import docling_document, marker_document
from knowledge.marker_parser import marker_blocks
from knowledge.run_log import record_event
from knowledge.storage import Database


def run_pending(settings: Settings) -> None:
...
def drain_queue(database: Database, root: Path) -> None:
    """Process one expensive job at a time while keeping failures inspectable."""
    while True:
        with database.transaction() as ledger:
            job = claim_job(ledger)
        if job is None:
            return
        execute_job(database, job, root)


def execute_job(database: Database, job: dict, root: Path) -> None:
    """Commit a completed snapshot or retain a bounded, classified failure."""
    audit = root / "runs" / "extraction" / job["request_hash"]
    record_event(audit, "extraction_started", source=str(job["source_id"]))
    try:
        snapshot = process_source(database, job, root, audit)
        with database.transaction() as ledger:
            save_snapshot(ledger, job["request_hash"], snapshot)
        record_event(audit, "extraction_succeeded", blocks=len(snapshot.blocks))
    except Exception as error:
        transient = isinstance(error, (URLError, TimeoutError, subprocess.TimeoutExpired))
        with database.transaction() as ledger:
            fail_job(ledger, job["request_hash"], str(error), transient)
        record_event(audit, "extraction_failed", error=str(error), transient=transient)


def process_source(database: Database, job: dict, root: Path, audit: Path) -> DocumentSnapshot:
    """Resolve pinned Zotero bytes before extracting outside the transaction."""
    reference = Reference(entity_id=job["source_id"], revision=job["source_revision"])
    with database.transaction() as ledger:
        record = ledger.get(reference.entity_id, reference.revision)
        attachment = sources.zotero_reference(ledger, record)
    if not isinstance(record.payload, Source):
...
def extract_snapshot(
    ...
    document = docling_document(pdf, staging / "docling", root)
    primary = [
        block
        for block in parse_document(document)
        if block.page is None or block.page in clean_pages
    ]
    sizes = page_sizes(document)
    ...
    blocks = reconcile_blocks(primary, candidates, scan_pages)
...
def read_selected_pages(
    ...
    for page in sorted(pages):
        output = marker_document(pdf, page, staging / f"marker-{page}", root)
        candidates.extend(marker_blocks(output, page, sizes[page]))
    return candidates


def coverage_issues(blocks: list[DocumentBlock], clean_pages: dict[int, int]) -> list[str]:
