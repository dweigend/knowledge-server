"""Compose source articles from Zotero, immutable extraction and existing knowledge."""
...
from knowledge import document_extraction, review, sources, zotero
from knowledge.contracts import Assessment, Claim, Evidence, Note, Record, ZoteroReference
from knowledge.document_contracts import DocumentBlock, DocumentSnapshot
from knowledge.storage import Database, Ledger, Missing
...
def compose_article(
    database: Database, record: Record, extraction_revision: int | None = None
) -> dict:
    ...
    if extraction_revision is not None and snapshot is None:
        raise Missing("Requested extraction revision is unavailable")
...
def read_citation(reference: ZoteroReference) -> dict:
...
def related_knowledge(ledger: Ledger, source_record: Record) -> list[dict]:
    ...
    for record in records:
        ...
        if isinstance(payload, Assessment) and payload.claim in claim_references:
            selected.append(record)
        if isinstance(payload, Note) and note_uses_source(ledger, record, source_record):
            selected.append(record)
    return [*claims, *[knowledge_entry(ledger, record, source_record) for record in selected]]


def note_uses_source(ledger: Ledger, note: Record, source: Record) -> bool:
...
def knowledge_entry(ledger: Ledger, record: Record, source: Record) -> dict:
    """Create a compact link while leaving full analysis on its existing record page."""
    title = record_title(record)
    if isinstance(record.payload, Assessment):
...
def source_claims(ledger: Ledger, records: list[Record], source_record: Record) -> list[dict]:
...
def eligible_overview(ledger: Ledger, record: Record, source_record: Record) -> bool:
    """Require human review and source-specific evidence citations in every summary paragraph."""
    note = record.payload
    if not isinstance(note, Note) or note.kind != "source":
        return False
    ...
    for reference in note.references:
        ...
        if isinstance(payload, Evidence) and payload.source == source_record.reference():
...
def present_blocks(record: Record, snapshot: DocumentSnapshot | None) -> list[dict]:
...
def table_rows(block: DocumentBlock) -> list[list]:
...
def pdf_location(
    record: Record, page: int, variant: str = "original", extraction_revision: int | None = None
) -> str:
...
def present_relationships(snapshot: DocumentSnapshot | None) -> list[dict]:
...
def preview_image(
    ...
    with database.transaction() as ledger:
        ...
        snapshot = document_extraction.get_snapshot(ledger, record.reference(), extraction_revision)
    if snapshot is None:
        raise Missing("Source has no structured extraction")
    ...
    if block is None or block.page is None or block.kind not in {"figure", "picture", "table"}:
        raise Missing("Unknown figure or table")
    pdf = zotero.verified_pdf(snapshot.zotero, "original")
    return render_region(pdf, block)


def render_region(pdf: Path, block: DocumentBlock) -> bytes:
...
def linked_text(block: DocumentBlock, snapshot: DocumentSnapshot) -> Markup:
...
def outline_entries(snapshot: DocumentSnapshot | None) -> list[dict]:
...
def processing_state(ledger: Ledger, record: Record) -> dict | None:
...
def read_source_citation(database: Database, record: Record) -> dict:
