"""Convert Docling JSON into ordered source blocks without a parser runtime dependency."""

from collections.abc import Iterator
from typing import Any

from knowledge.document_checks import table_issues
from knowledge.document_contracts import DocumentBlock, DocumentLocation, TableCell
...
def parse_document(document: dict[str, Any]) -> list[DocumentBlock]:
...
def _assign_section_kinds(blocks: list[DocumentBlock], body_references: set[str]) -> None:
    section_kind = ""
    for block in blocks:
...
def _parse_location(provenance: dict[str, Any], document: dict[str, Any]) -> DocumentLocation:
    page = int(provenance["page_no"])
    bounding_box = provenance.get("bbox")
    if not bounding_box:
        return DocumentLocation(page=page, region=None)
    ...
    return DocumentLocation(page=page, region=(left, top, right, bottom))


def _parse_cell(cell: dict[str, Any]) -> TableCell:
    return TableCell(
...
def _block_issues(block: DocumentBlock) -> list[str]:
    issues = []
    ...
    if block.kind == "table":
        issues.extend(table_issues(block))
    return issues
