"""Persist immutable document snapshots and the fixed extraction queue."""
...
from knowledge.document_contracts import DocumentAnnotation, DocumentSnapshot
from knowledge.storage import Conflict, Ledger, Missing
...
def get_snapshot(
    ledger: Ledger, source: Reference, revision: int | None = None
    ...
    return DocumentSnapshot.model_validate(row["payload"]) if row else None


def request_extraction(ledger: Ledger, source: Reference, retry_failed: bool = False) -> str:
...
def claim_job(ledger: Ledger) -> dict | None:
...
def recover_jobs(ledger: Ledger) -> None:
...
def fail_job(ledger: Ledger, request_hash: str, error: str, transient: bool) -> None:
...
def save_snapshot(ledger: Ledger, request_hash: str, snapshot: DocumentSnapshot) -> None:
...
def append_snapshot(ledger: Ledger, request_hash: str, snapshot: DocumentSnapshot) -> None:
...
def validate_snapshot_job(ledger: Ledger, request_hash: str, snapshot: DocumentSnapshot) -> None:
    ...
    if not isinstance(source, Source) or source.sha256 != snapshot.pdf_sha256:
...
def annotate_document(ledger: Ledger, annotation: DocumentAnnotation) -> int:
    ...
    if snapshot is None:
        raise Missing("No extraction to annotate")
    if snapshot.revision != annotation.expected_revision:
        raise Conflict("Extraction changed before annotation")
    apply_annotations(snapshot, annotation)
    append_snapshot(ledger, identity, snapshot)
    return snapshot.revision


def apply_annotations(snapshot: DocumentSnapshot, annotation: DocumentAnnotation) -> None:
