"""Convert GROBID TEI into a provider-neutral paper without fabricated source spans."""

import re
from collections import Counter
from xml.etree import ElementTree as ET

from knowledge.paper_contracts import (
    PaperCitation,
    PaperDocument,
    PaperMetadata,
    PaperReference,
...
def bibliography_metadata(element: ET.Element) -> PaperMetadata:
...
def paper_metadata(root: ET.Element) -> PaperMetadata:
    ...
    metadata = bibliography_metadata(source) if source is not None else PaperMetadata()
...
def paper_references(root: ET.Element) -> list[PaperReference]:
...
def paper_citations(root: ET.Element, references: list[PaperReference]) -> list[PaperCitation]:
...
def document_markdown(
    root: ET.Element, metadata: PaperMetadata, references: list[PaperReference]
) -> str:
...
def parse_paper_tei(raw_tei: str) -> PaperDocument:
    ...
    sections = [
        PaperSection(title=element_text(head), coordinates=head.get("coords"))
...
def extraction_warnings(
    metadata: PaperMetadata, references: list[PaperReference], citations: list[PaperCitation]
) -> list[str]:
