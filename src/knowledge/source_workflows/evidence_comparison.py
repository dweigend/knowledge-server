"""Compare a claim against bounded cross-source evidence candidates.

The workflow retrieves source passages, validates proposals, and links accepted
evidence while keeping overall assessment separate.
"""
...
from knowledge.knowledge_base import claim_evidence_records, knowledge_service
...
class Comparison(models.Contract):
    """Represent a bounded set of proposed cross-source relations."""

    relations: list[models.Evidence] = Field(max_length=3)
...
def candidate_packet(
    ...
    relations = [
        record.payload for record in current_evidence if isinstance(record.payload, models.Evidence)
    ]
...
def source_candidate(record: models.Record, source: models.Source, keywords: set[str]) -> dict:
    ...
    return {
        ...
        "bibliography": zotero_client.get_bibliography(source).model_dump(mode="json"),
...
def validate_comparison(
    ...
    *,
    ...
    batch_id: str,
    claim_reference: models.Reference,
    ...
    for relation in comparison.relations:
        ...
        assert isinstance(source, models.Source)
        try:
            relation.page, relation.quote = pdf_text_extraction.locate_passage(
                source.pages,
                relation.page,
                relation.quote,
            )
...
def compare_claim(
    ...
    with application.database.transaction() as ledger:
        current_evidence = claim_evidence_records.records_for_claim(
...
def prepare_comparison(
    ...
    current_evidence: list[models.Record],
    source_records: list[models.Record],
) -> knowledge_service.CompareEvidence:
    ...
    if prepared.exists():
        return knowledge_service.CompareEvidence.model_validate_json(prepared.read_text())
    packet, selection = candidate_packet(claim, current_evidence, source_records)
    comparison = structured_generation.generate(
        prompt_registry.load_prompt("compare"),
        packet,
        Comparison,
    ...
    command = knowledge_service.CompareEvidence(
...
def assess_compared_claim(
    ...
    assessment = structured_generation.generate(
        prompt_registry.load_prompt("assess"),
        packet,
        models.Assessment,
        batch_root / "proposals",
    )
    ...
    if not done:
        application.assess(
            receipt_id,
            batch_id,
            knowledge_service.AssessmentCommand(
...
def assessment_packet(claim: models.Record, relations: list[models.Record], search: dict) -> str:
