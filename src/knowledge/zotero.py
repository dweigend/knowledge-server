"""Read literature and manage stored PDF attachments through Zotero's local API."""
...
from knowledge.contracts import Bibliography, LegacySource, Source, ZoteroReference
...
def item_data(reference: ZoteroReference) -> dict:
...
def get_bibliography(source: Source | ZoteroReference) -> Bibliography:
    ...
    if isinstance(source, LegacySource):
        ...
        reference = source if isinstance(source, ZoteroReference) else source.zotero
...
def verified_pdf(reference: ZoteroReference, variant: str) -> Path:
...
def import_sources(
    bibliography: Bibliography, original_pdf: Path, clean_pdf: Path, batch_id: str
) -> ZoteroReference:
...
def find_or_import_item(
    bibliography: Bibliography, clean_pdf: Path, source_hash: str, batch_id: str
) -> str:
...
def import_ris(bibliography: Bibliography, clean_pdf: Path, tag: str) -> None:
...
def article_citation(reference: ZoteroReference) -> dict:
