"""Run heavyweight PDF extraction tools in bounded subprocess groups.

Each invocation uses private scratch space, controlled environment variables,
...
def run_tool(arguments: list[str], directory: Path, runtime_root: Path) -> None:
    ...
    started = monotonic()
    events.record_event(directory, "tool_started", tool=arguments[0], unit=unit)
    try:
        ...
        events.record_event(
            directory, "tool_finished", tool=arguments[0], seconds=monotonic() - started
        )
